...
// ✅ FIX: Group machines by coordinates instead of random jitter
const clusteredMachines = computed(() => {
  const groups: Record<string, any[]> = {};

  props.machines.forEach(m => {
      // Rounding to 4 decimal places groups machines within ~11 meters
  ...
  return Object.values(groups).map(group => {
      // A cluster is "Online" if AT LEAST one machine in it is online
      ...
      return {
          id: group[0].id, // Use first ID as key
      ...
      >
        <l-popup>
          <div class="max-h-64 overflow-y-auto custom-scrollbar">
            ...
            >
                <strong class="block text-slate-800 text-sm mb-0.5">{{ m.device_name || 'Unnamed' }}</strong>
                ...
                <div class="text-xs font-bold flex items-center gap-1" 
                    :class="(m.is_online || m.is_active || m.status === 'ONLINE' || m.isOnline) ? 'text-green-600' : 'text-red-500'">
                    <span class="w-1.5 h-1.5 rounded-full" 
                        :class="(m.is_online || m.is_active || m.status === 'ONLINE' || m.isOnline) ? 'bg-green-500' : 'bg-red-500'">
                    </span>
                    {{ (m.is_online || m.is_active || m.status === 'ONLINE' || m.isOnline) ? 'ONLINE' : 'OFFLINE' }}
                </div>
            </div>
          </div>
        </l-popup>
