...
router.beforeEach(async (to, _from, next) => {
  ...
  if (session) {
    // Check Admin Role
    console.log('🔍 Router: Checking admin role for:', session.user.email);
    
    // Try with .single() first - case insensitive
    ...
    console.log('📊 Router admin query result:', { admin, error });

    // If 406/PGRST116, try fallback with exact case
    if (error && (error.code === '406' || error.code === 'PGRST116' || error.message.includes('0 rows'))) {
      console.warn('⚠️ No match with ilike, trying exact case...');
      const { data: fallbackAdmin } = await supabase
        .from('app_admins')
        .select('role')
        .eq('email', session.user.email!);
      
      if (!fallbackAdmin || fallbackAdmin.length === 0) {
        await supabase.auth.signOut();
        next({ name: 'login' });
        return;
      }
      
      const fallbackRec = fallbackAdmin[0];
      if (!fallbackRec) {
        await supabase.auth.signOut();
        next({ name: 'login' });
        return;
      ...
      // Check role
      if (requiresSuperAdmin && fallbackRec.role !== 'SUPER_ADMIN') {
    ...
    if (error) {
      console.error('❌ Router admin query error:', error);
      await supabase.auth.signOut();
      next({ name: 'login' });
      return;
    }
    ...
    if (!admin) {
      console.warn('⚠️ No admin record found in router');
      await supabase.auth.signOut();
      next({ name: 'login' });
      return;
    }
